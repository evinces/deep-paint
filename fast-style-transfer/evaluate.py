...
# def ffwd_video(path_in, path_out, checkpoint_dir, device_t='/gpu:0',
#                batch_size=4):
...
#                 video_writer.write_frame(np.clip(_preds[i], 0, 255).astype(
#                                                                     np.uint8))
...
def main():
    ...
    if not os.path.isdir(opts.in_path):
        ...
        if os.path.exists(opts.out_path) and os.path.isdir(opts.out_path):
            print('   TRUE os.path.exists(opts.out_path) and os.path.isdir(opts.out_path)')

            out_path = os.path.join(opts.out_path,
                                    os.path.basename(opts.in_path))
        else:
            print('   FLASE os.path.exists(opts.out_path) and os.path.isdir(opts.out_path)')
